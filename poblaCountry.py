"""
Proyecto creado por Bogdan Rivera 
Este trabajo se enfoca en leer un archivo de tipo csv para
poder encontrar un país en específico. Una vez teniendo eso 
...
def lee_csv(path):
    with open(path, 'r') as file:
        # Lee el archivo CSV con delimitador en comas (,)
        reader = csv.reader(file, delimiter=',')
        # Ubica únicamente la primera fila y la guarda en un arreglo
        header = next(reader)
        paises = []
        for row in reader:
            item = zip(header, row)  # Contatena el header con las filas
            # Realiza un cambio a diccionarios
            dict_paises = {country: i for country, i in item}
